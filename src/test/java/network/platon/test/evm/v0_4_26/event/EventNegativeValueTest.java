...
public class EventNegativeValueTest extends ContractPrepareTest {
    ...
    public void test() {
        prepare();
        try {
            Eventer eventer = Eventer.deploy(web3j, transactionManager, provider).send();
            ...
            TransactionReceipt transactionReceipt = eventer.load(contractAddress, web3j, transactionManager, provider).getEvent().send();
