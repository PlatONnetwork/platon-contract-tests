package network.platon.test.evm.v0_8_2.event;

import com.platon.abi.solidity.datatypes.generated.Uint256;
import com.platon.protocol.core.methods.response.TransactionReceipt;
import network.platon.autotest.junit.annotations.DataSource;
import network.platon.autotest.junit.enums.DataSourceType;
import network.platon.contracts.evm.v0_7_6.EventTypeContract;
...
public class EventTypeContractTest extends ContractPrepareTest {
    ...
    @DataSource(type = DataSourceType.EXCEL, file = "test.xls", sheetName = "testOneDimensionalArray",
            author = "albedo", showName = "event.EventTypeContractTest-一维数组类型", sourcePrefix = "evm/0.7.6")
    public void testOneDimensionalArray() {
        try {
            prepare();
            EventTypeContract eventTypeContract = EventTypeContract.deploy(web3j, transactionManager, provider).send();
    ...
    @DataSource(type = DataSourceType.EXCEL, file = "test.xls", sheetName = "testTwoDimensionalArray",
            author = "albedo", showName = "event.EventTypeContractTest-二维数组类型", sourcePrefix = "evm/0.7.6")
    ...
    @DataSource(type = DataSourceType.EXCEL, file = "test.xls", sheetName = "testStr",
            author = "albedo", showName = "event.EventTypeContractTest-字符串", sourcePrefix = "evm/0.7.6")
    public void testStr() {
        try {
            prepare();
            EventTypeContract eventCallContract = EventTypeContract.deploy(web3j, transactionManager, provider).send();
    ...
    @DataSource(type = DataSourceType.EXCEL, file = "test.xls", sheetName = "testEnum",
            author = "albedo", showName = "event.EventTypeContractTest-枚举", sourcePrefix = "evm/0.7.6")
