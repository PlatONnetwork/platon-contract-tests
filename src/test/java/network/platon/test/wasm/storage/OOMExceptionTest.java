...
public class OOMExceptionTest extends WASMContractPrepareTest {
    ...
    public void test() {
        ...
        try {
            OOMException oomException = OOMException.deploy(web3j, transactionManager, provider).send();
            ...
            TransactionReceipt transactionReceipt = OOMException.load(contractAddress, web3j, transactionManager, provider).memory_limit().send();
