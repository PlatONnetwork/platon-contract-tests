...
public class DonateTest extends WASMContractPrepareTest {
    ...
    public void testDonate() {
        ...
        Uint128 _maxVonAmount = Uint128.of(Convert.toVon(new BigDecimal(10000), Convert.Unit.KPVON).toBigInteger());
        Uint128 _maxNumDonors = Uint128.of(100000);

        try {
            System.out.println(_openingTime);
            System.out.println(_closingTime);
            Donate donate = Donate.deploy(web3j, transactionManager, provider, _charity, _openingTime, _closingTime, _minVonAmount, _maxVonAmount, _maxNumDonors).send();
