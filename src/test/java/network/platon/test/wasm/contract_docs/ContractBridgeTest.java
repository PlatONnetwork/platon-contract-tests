...
//            WasmAddress wasmAddress2 = new WasmAddress("lax1fyeszufxwxk62p46djncj86rd553skpptsj8v6");
            ...
            t.sendFunds(contractAddress, new BigDecimal(100), Convert.Unit.KPVON, provider.getGasPrice(), provider.getGasLimit()).send();
    ...
    public void testForeignBridge() {
        try {
            ...
            ForeignBridge contract = ForeignBridge.deploy(web3j, transactionManager, provider,
                    Xuint128.ONE, new WasmAddress[]{new WasmAddress(credentials.getAddress())}, Xuint128.ONE).send();
            String contractAddress = contract.getContractAddress();
            String transactionHash = contract.getTransactionReceipt().get().getTransactionHash();
            ...
            // deposit
            TransactionReceipt depositTr = contract.deposit(new WasmAddress(credentials.getAddress()), Uint128.of(1000000), new byte[]{}).send();
