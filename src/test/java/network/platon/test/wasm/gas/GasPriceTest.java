...
public class GasPriceTest extends WASMContractPrepareTest {
    ...
    public void test() {
        ...
        try {
            prepare();
            GasPrice gasPrice = GasPrice.deploy(web3j, transactionManager, provider).send();
            ...
            gasPrice = GasPrice.load(contractAddress, web3j, transactionManager, provider);
...
//                    transactionReceipt = gasPrice.platonBlockHash(Int64.of(BigInteger.TEN)).send();
                ...
                case 27:{
                    //跨合约普通调用
                    GasPrice newGasPrice = GasPrice.deploy(web3j, transactionManager, provider).send();
                ...
                case 29:{
                    ...
                    //合约销毁，地址未使用过
                    gasPrice = GasPrice.deploy(web3j, transactionManager, provider).send();
                    wasmAddress = new WasmAddress("lat1uqug0zq7rcxddndleq4ux2ft3tv6dqljwjkzds");
                    transactionReceipt = gasPrice.platonDestory(wasmAddress).send();
                    gas = this.getGasValue(transactionReceipt, gasPrice, "platon_destroy");
                    collector.logStepPass("gas of platonDestory: " + gas);
                    collector.assertTrue((Math.abs(5000 - gas.intValue())) < 100);
