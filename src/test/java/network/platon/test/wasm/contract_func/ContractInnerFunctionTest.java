...
public class ContractInnerFunctionTest extends WASMContractPrepareTest {
    ...
    public void testFunctionContract_02() {
        ...
        try {
            ...
            String latAddress = origin.getAddress().startsWith("lat") ? origin.getAddress() : PlatonAddressChangeUtil.encode("lat", convertBits(Numeric.hexStringToByteArray(origin.getAddress()), 8, 5, true));
            collector.assertEqual(credentials.getAddress(), latAddress);

            // test: transfer
            String toAddress = "0x250b67c9f1baa47dafcd1cfd5ad7780bb7b9b196";
            toAddress = Bech32.addressEncode(NetworkParameters.getHrp(), toAddress);
            long amount = 1;
            Transfer t = new Transfer(web3j, transactionManager);
            t.sendFunds(contractAddress, new BigDecimal(amount), Convert.Unit.KPVON, provider.getGasPrice(), provider.getGasLimit()).send();
            ...
            receiveAddr = Bech32.addressEncode(NetworkParameters.getHrp(), receiveAddr);
