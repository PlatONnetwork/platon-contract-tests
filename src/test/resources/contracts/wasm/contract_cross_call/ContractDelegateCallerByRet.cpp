...
CONTRACT delegate_caller_byret : public platon::Contract {
    public:
        ACTION void init(){}
        ...
        ACTION void callFeed(std::string target_address, uint64_t gasValue) {
            ...
            if(result.second){
                ...
                DEBUG("delegate_caller_byret call receiver_byret info has successed!")
            } else {
                status.self() = 1; //failed

                DEBUG("delegate_caller_byret call receiver_byret info has failed!")
            }
            }
        }
...
PLATON_DISPATCH(delegate_caller_byret, (init)(callFeed)(get_status))